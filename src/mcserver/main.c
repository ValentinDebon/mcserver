...
struct mcserver_command {
	const char *name;
	void (*run)(const struct mcserver_args *);
};
...
mcserver_usage(const char *mcservername) {
	fprintf(stderr, "usage: %s [-cache <duration>] [-manifest-url <url>] [-snapshot <id> | -release <id> | -beta <id> | -alpha <id>] install\n"
		            "       %s [-jvm <java>] [-snapshot <id> | -release <id> | -beta <id> | -alpha <id>] launch\n"
		            "       %s -help\n",
		mcservername, mcservername, mcservername);
	exit(EXIT_FAILURE);
}
...
main(int argc, char **argv) {
	...
	const struct mcserver_command *current = commands, * const commandsend = commands + sizeof(commands) / sizeof(*commands);
